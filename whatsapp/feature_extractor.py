...
class FeatureExtractor:
    def __init__(self, time_window, video_ptype, max_packets_in_time_window):
        self.time_window = time_window
        self.video_ptype = video_ptype
    ...
    def extract_summary_stats(self, data_group_by: pd.DataFrame, feature: str, window_end_time_stamps: list, reduce_file_weight=False):
        ...
        min_feat = features_df.min(axis=1)
        mean_feat = features_df.mean(axis=1)
        std_feat = features_df.std(axis=1)
        q1_feat = features_df.quantile(0.75, axis=1)
        q2_feat = features_df.quantile(0.5, axis=1)
        q3_feat = features_df.quantile(0.25, axis=1)
