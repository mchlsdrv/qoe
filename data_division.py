...
def build_test_datasets(data: pd.DataFrame, n_test_sets, test_set_proportion: float, root_save_dir: pathlib.Path):
    """
    Divides the data into n_test_sets train-test datasets each with proportion of
    ...
    * Each test dataset is chosen to not include items from other test sets
    ...
    :param test_set_proportion: The proportion by which to divide the data
    ...
    # - Create a n_test_sets train-test sets
    for test_set_idx in range(n_test_sets):
        ...
        test_set_save_dir = root_save_dir / f'ablation/test_{test_set_idx}'
...
DATA_PATH = pathlib.Path('/data/zoom/encrypted_traffic/data_no_nan.csv')
DATA_SET = pd.read_csv(DATA_PATH)
SAVE_DIR = pathlib.Path('/data/zoom/encrypted_traffic')
